...
cmdHandlers:Record<string,(msg:MessageContext)=>Promise<void>>={checkapi:async(msg)=>{
  ...
  // ── list / del / save / check ──
  if(sub==="list"){const keys=await lk();if(!keys.length){await msg.edit({text:html`📭 未保存\n<code>${mp}checkapi save &lt;name&gt; &lt;key&gt;</code>`});return;}const lines=[`🔑 已保存 (${keys.length}):`];for(const k of keys)lines.push(`  • <b>${k.name}</b>: ${mk(k.key)} (${k.provider||"auto"})${k.baseUrl?` [${(()=>{try{return new URL(k.baseUrl).hostname}catch{return k.baseUrl}})()}]`:""}`);await msg.edit({text:html`${lines.join("\n")}`});return;}
