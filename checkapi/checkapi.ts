...
cmdHandlers:Record<string,(msg:MessageContext)=>Promise<void>>={checkapi:async(msg)=>{
  ...
  // ── list / del / save / check ──
  if(sub==="list"){const keys=await lk();if(!keys.length){await msg.edit({text:html`📭 未保存\n<code>${mp}checkapi save &lt;name&gt; &lt;key&gt;</code>`});return;}const lines=[`🔑 已保存 (${keys.length}):`];for(const k of keys)lines.push(`  • <b>${k.name}</b>: ${mk(k.key)} (${k.provider||"auto"})${k.baseUrl?` [${new URL(k.baseUrl).hostname}]`:""}`);await msg.edit({text:html`${lines.join("\n")}`});return;}
  if(sub==="del"||sub==="delete"){const name=parts[1];if(!name){await msg.edit({text:html`❌ <code>${mp}checkapi del &lt;name&gt;</code>`});return;}const keys=await lk();const idx=keys.findIndex(k=>k.name===name);if(idx===-1){await msg.edit({text:html`❌ 未找到 <b>${name}</b>`});return;}keys.splice(idx,1);await sk(keys);await msg.edit({text:html`✅ 已删除 <b>${name}</b>`});return;}
  if(sub==="save"){let name:string|undefined,key:string|undefined,baseUrl:string|undefined;if(parts.length>=4&&isUrl(parts[2])){name=parts[1];baseUrl=nu(parts[2]);key=parts[3];}else{name=parts[1];key=parts[2];baseUrl=parts[3];}if(!name||!key){await msg.edit({text:html`❌ <code>${mp}checkapi save &lt;name&gt; &lt;key|url&gt; &lt;key|url&gt;</code>\n示例: <code>${mp}checkapi save oai sk-xxx</code>`});return;}const keys=await lk();const info=dp(key,baseUrl);const entry:SK={name,key,baseUrl,provider:info.provider,addedAt:Date.now()};const idx=keys.findIndex(k=>k.name===name);if(idx>=0)keys[idx]=entry;else keys.push(entry);await sk(keys);await msg.edit({text:html`✅ ${idx>=0?"已更新":"已保存"} <b>${name}</b> (${info.displayName})\n<code>${mp}checkapi ${name}</code> 检测`});return;}
  if(sub==="check"){const target=parts[1]||"all";if(target==="all"){const keys=await lk();if(!keys.length){await msg.edit({text:html`📭 未保存`});return;}await msg.edit({text:html`🔍 检测 ${keys.length} 个...`});const results:string[]=[];for(const k of keys){results.push(`\n━━━ <b>${k.name}</b> ━━━`);const info=dp(k.key,k.baseUrl);results.push(...(await fcv2(info.provider,k.key,k.baseUrl||info.baseUrl)));}await msg.edit({text:html`${results.join("\n")}`});return;}const keys=await lk();const found=keys.find(k=>k.name===target);if(!found){await msg.edit({text:html`❌ 未找到 <b>${target}</b>`});return;}await msg.edit({text:html`🔍 <b>${target}</b>...`});const info=dp(found.key,found.baseUrl);const results=await fcv2(info.provider,found.key,found.baseUrl||info.baseUrl);await msg.edit({text:html`${results.join("\n")}`});return;}

  // ── models / ask / speed ──
  if(sub==="models"){const input=parts[1];if(!input){await msg.edit({text:html`❌ <code>${mp}checkapi models &lt;key|name&gt;</code>`});return;}const keys=await lk();const found=keys.find(k=>k.name===input);const key=found?found.key:input;const info=dp(key,found?.baseUrl);await msg.edit({text:html`🔍 ${info.displayName} 模型列表...`});const result=await lmf(info.provider,key,info.baseUrl);await msg.edit({text:html`${result}`});return;}
  if(sub==="ask"){const input=parts[1];const question=parts.slice(2).join(" ");if(!input){await msg.edit({text:html`❌ <code>${mp}checkapi ask &lt;key|name&gt; &lt;问题&gt;</code>`});return;}const keys=await lk();const found=keys.find(k=>k.name===input);const key=found?found.key:input;const info=dp(key,found?.baseUrl);const prompt=question||"say hello";await msg.edit({text:html`💬 ${info.displayName}: "${prompt}" ...`});const chat=await ct(info.provider,key,info.baseUrl,prompt);if(chat.ok){const l=[`💬 <b>${info.displayName}</b>`,`🤖 <code>${chat.model}</code> | 🕐 ${chat.elapsedMs}ms`,`📝 ${chat.text}`];if(chat.usage)l.push(`📊 Tok: 入${chat.usage.prompt} 出${chat.usage.completion} 计${chat.usage.total}`);if(chat.headers)l.push(fh(chat.headers));await msg.edit({text:html`${l.join("\n")}`});}else{await msg.edit({text:html`❌ (${chat.elapsedMs||"?"}ms): ${chat.error}`});}return;}
  ...
  const info=dp(key,baseUrl);
  await msg.edit({text:html`🔍 <b>${label}</b> (${info.displayName}) 全检中...`});
