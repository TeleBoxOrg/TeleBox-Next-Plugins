...
class AutoDelPlugin extends Plugin {
  ...
  description: string = `🕒 <b>定时自动删除消息</b>\n\n
<b>命令</b>\n
...
<b>示例</b>\n
• <code>${mainPrefix}autodel 30s</code>\n
• <code>${mainPrefix}autodel 5 分钟 global</code>（设置全局）\n\n
<b>⚠️ 安全说明</b>\n
• 只会删除您自己发送的消息\n
• 最小删除时间为5秒`;
  ...
  private async handleAutoDel(msg: MessageContext): Promise<void> {
    const args = msg.text.split(" ").slice(1);
    
    if (args.length === 0 || args[0] === "h") {
      await msg.edit({
        text: html(this.getHelpText())
      });
      return;
    }
  ...
  private async showAutoDelSettings(msg: MessageContext): Promise<void> {
  ...
  private async cancelAutoDelSetting(msg: MessageContext, isGlobal: boolean): Promise<void> {
    const chatId = isGlobal ? "0" : msg.chat.id.toString();
  ...
  private async setAutoDelSetting(msg: MessageContext, seconds: number, isGlobal: boolean): Promise<void> {
    const chatId = isGlobal ? "0" : msg.chat.id.toString();
